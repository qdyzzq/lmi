<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::table('field_offices', function (Blueprint $table) {
            // Only add new columns if they don't exist yet
            if (!Schema::hasColumn('field_offices', 'position_title_id')) {
                $table->foreignId('position_title_id')
                      ->nullable()
                      ->after('office_type')
                      ->constrained('position_titles')
                      ->nullOnDelete();
            }

            if (!Schema::hasColumn('field_offices', 'persons_name')) {
                $table->string('persons_name')
                      ->nullable()
                      ->after('position_title_id')
                      ->comment('The actual person holding the position');
            }
        });
    }

    public function down(): void
    {
        Schema::table('field_offices', function (Blueprint $table) {
            if (Schema::hasColumn('field_offices', 'position_title_id')) {
                $table->dropForeign(['position_title_id']);
                $table->dropColumn('position_title_id');
            }

            if (Schema::hasColumn('field_offices', 'persons_name')) {
                $table->dropColumn('persons_name');
            }
        });
    }
};