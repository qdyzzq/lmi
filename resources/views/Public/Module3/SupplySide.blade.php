...
<head>
    ...
    <style>
        ...
        @media (max-width: 767px) {
            ...
            .pie-modal-legend-side {
                display: none !important;
            }

            .pie-modal-legend-bottom {
        ...
        @media (max-width: 640px) {
            ...
            .modal-header-inner .modal-close-group {
                align-self: flex-end;
            }
        ...
        .enrollment-trend-mobile {
            display: none;
        }

        .enrollment-trend-desktop {
            display: block;
        }
        ...
        .discipline-enrollment-mobile {
            display: none;
        }

        .discipline-enrollment-desktop {
            display: block;
        }
...
<body x-data="licensureChartData()" class="bg-slate-100 min-h-screen">
    ...
    <div class="relative w-full h-[380px] sm:h-[420px] md:h-[700px] lg:h-[900px] overflow-hidden">
        <div class="absolute inset-0">
            <img src="{{ asset('images/ARD.webp') }}" alt="Assistant Regional Director"
        ...
        <!-- Scroll Indicator -->
        <div
            class="absolute bottom-6 sm:bottom-16 left-1/2 transform -translate-x-1/2 z-20 scroll-indicator animate-bounce">
            <a href="#kpi-section" class="flex flex-col items-center cursor-pointer group"
                @click.prevent="() => {
        ...
        <div class="px-2 py-3 sm:p-5">
            <div class="max-w-screen-xl mx-auto px-0 sm:px-4 space-y-6">
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="text-sm text-yellow-700 mt-1">
                                This analysis currently reflects student data from <strong>Higher Education Institutions
                                    (HEIs)</strong> under CHED. It <strong>does not</strong> yet include workforce
                                supply from <strong>Technical-Vocational (TESDA)</strong> or <strong>Senior High School
                                    (DepEd)</strong> graduates. Future dashboard updates aim to integrate these
                                additional data sources to provide a comprehensive view of the total regional labor
                                supply.
                            </p>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4" id="kpi-section">
                    ...
                    <div
                        class="group bg-white rounded-2xl p-5 sm:p-8 border-l-4 border-blue-500 shadow-lg hover:shadow-xl transition-all duration-200 hover:-translate-y-0.5 relative overflow-hidden kpi-card">
                        <div x-show="loadingGraduationRate || loadingLatestEnrollment"
                            class="absolute inset-0 bg-white/80 rounded-2xl flex items-center justify-center z-10">
                            <div
                                class="animate-spin rounded-full h-10 w-10 border-2 border-slate-200 border-t-slate-500">
                            </div>
                        </div>
                        ...
                        <div class="relative">
                            <div class="flex items-start justify-between mb-5">
                                <div>
                                    <p class="text-xs font-bold text-blue-500 uppercase tracking-widest mb-1">Total
                                        Enrollees</p>
                                    <p class="text-xs text-slate-400">Latest enrollment data</p>
                                </div>
                                <div
                                    class="bg-blue-100 p-3.5 rounded-full flex items-center justify-center group-hover:bg-blue-600 transition-colors">
                                    <svg class="w-8 h-8 text-blue-600 group-hover:text-white transition-colors"
                            ...
                            <p class="text-4xl sm:text-6xl font-black text-slate-800 mb-3 tracking-tight kpi-number"
                                x-text="formatNumber(latestEnrollmentTotal || 0)">0</p>
                            <div class="flex items-center gap-3">
                                <div class="w-2 h-2 rounded-full bg-blue-400"></div>
                                <p class="text-sm text-slate-500 font-medium"
                                    x-text="latestEnrollmentYear ?? 'No data'">No data</p>
                            </div>
                        </div>
                        <!-- Tooltip -->
                        <div class="absolute top-5 right-5" x-data="{ showTooltip: false }">
                            ...
                            <div x-show="showTooltip" x-transition
                                class="absolute right-0 top-6 w-64 p-3 bg-slate-800 text-white text-xs rounded-lg shadow-xl z-50">
                                Total number of students currently enrolled across all disciplines for the latest
                    ...
                    <div
                        class="group bg-white rounded-2xl p-5 sm:p-8 border-l-4 border-violet-500 shadow-lg hover:shadow-xl transition-all duration-200 hover:-translate-y-0.5 relative overflow-hidden kpi-card">
                        <div x-show="loadingGraduationRate"
                            class="absolute inset-0 bg-white/80 rounded-2xl flex items-center justify-center z-10">
                            <div
                                class="animate-spin rounded-full h-10 w-10 border-2 border-slate-200 border-t-slate-500">
                            </div>
                        ...
                        <div class="relative">
                            <div class="flex items-start justify-between mb-5">
                                <div>
                                    <p class="text-xs font-bold text-violet-500 uppercase tracking-widest mb-1">
                                        Projected Graduates</p>
                                    <!-- Year + rate badge (replaces "Based on graduation rate") -->
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span class="text-xs font-semibold text-slate-500"
                                            x-text="graduationRateData.graduate_year ?? '—'"></span>
                                        <span
                                            class="text-xs font-bold text-violet-600 bg-violet-100 px-2.5 py-1 rounded-full"
                                            x-show="graduationRateData.graduation_rate"
                                            x-text="`${parseFloat(graduationRateData.graduation_rate).toFixed(2)}% rate`"></span>
                                    </div>
                                </div>
                                <div
                                    class="bg-violet-100 p-3.5 rounded-full flex items-center justify-center group-hover:bg-violet-600 transition-colors">
                                    <svg class="w-8 h-8 text-violet-600 group-hover:text-white transition-colors"
                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <p class="text-4xl sm:text-6xl font-black text-slate-800 mb-3 tracking-tight kpi-number"
                                x-text="formatNumber(graduationRateData.projected_graduates || 0)">0</p>
                        ...
                        <div class="absolute top-5 right-5" x-data="{ showTooltip: false }">
                            ...
                            <div x-show="showTooltip" x-transition
                                class="absolute right-0 top-6 w-64 p-3 bg-slate-800 text-white text-xs rounded-lg shadow-xl z-50">
                                Based on <span class="font-semibold"
                                    x-text="graduationRateData.enrollment_year ?? '—'"></span> enrollment data <span
                                    class="italic">(4 years ago)</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-6 rounded-2xl overflow-hidden shadow-lg" x-data="{ enrollmentOverviewExpanded: true }">
                    ...
                    <button @click="enrollmentOverviewExpanded = !enrollmentOverviewExpanded"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="text-left">
                                ...
                                <p class="text-xs text-slate-400">Discipline market share &amp; executive supply
                                    analysis
                                    <span x-show="!enrollmentOverviewExpanded" class="text-slate-500 ml-2">• Click to
                                        expand</span>
                                </p>
                            </div>
                        </div>
                        <svg class="w-5 h-5 text-slate-400 transition-transform duration-300"
                            :class="{ 'rotate-180': enrollmentOverviewExpanded }" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                    ...
                    <!-- Collapsible Content -->
                    <div x-show="enrollmentOverviewExpanded" x-transition:enter="transition ease-out duration-300"
                        ...
                        <!-- Panel Filter Bar -->
                        <div
                            class="flex flex-wrap items-center justify-end px-4 sm:px-6 py-3 bg-white border-b border-slate-200 gap-2 sm:gap-3">
                            <div
                                class="flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl px-3 py-1.5 shadow-sm">
                                ...
                                <select x-model="selectedEnrollmentProvince"
                            ...
                            <div
                                ...
                                <select x-model="selectedEnrollmentYear" @change="loadEnrollmentData()"
                                    class="text-sm font-bold text-slate-800 bg-transparent focus:outline-none cursor-pointer">
                        ...
                        <div class="flex flex-col gap-6 p-6">
                            ...
                            <div class="w-full bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
                                <div class="flex flex-col p-6">
                                    <div class="flex items-start gap-3 mb-4 flex-shrink-0">
                                        <div
                                            class="bg-gradient-to-br from-blue-500 to-indigo-600 p-2.5 rounded-xl shadow-lg">
                                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z">
                                    ...
                                    <div x-show="loadingExecutiveAnalysis"
                                        ...
                                        <div
                                            class="animate-spin rounded-full h-6 w-6 border-2 border-slate-200 border-t-slate-500">
                                        </div>
                                    </div>
                                    ...
                                    <div x-show="!loadingExecutiveAnalysis"
                                        ...
                                        style="max-height: 320px;" x-html="executiveAnalysisText">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div
                                class="w-full bg-slate-50 rounded-2xl shadow-xl border border-slate-200 p-6 relative overflow-hidden">
                                <div x-show="loadingPieChart"
                                    class="absolute inset-0 bg-white/75 flex items-center justify-center z-10 rounded-2xl backdrop-blur-sm">
                                    <div
                                        class="animate-spin rounded-full h-10 w-10 border-2 border-slate-200 border-t-slate-500">
                                    </div>
                                </div>
                                <div class="flex items-start justify-between gap-3 mb-4">
                                    <div class="flex items-start gap-3">
                                        <div
                                            class="bg-gradient-to-br from-purple-500 to-pink-600 p-2.5 rounded-xl shadow-lg">
                                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <!-- Expand Button -->
                                    <button @click="pieModalOpen = true"
                                ...
                                <div class="pie-inline-layout mt-6">
                                    ...
                                    <div class="w-56 pie-legend-col shrink-0 space-y-3">
                                        ...
                                        return entries
                                            ...
                                            <div class="flex items-start gap-2 min-w-0">
                                                ...
                                                <div class="min-w-0">
                                                    <div class="text-slate-700 font-medium text-sm leading-snug"
                                                        x-text="formatDisciplineName(item[0])"></div>
                                                    <div class="text-slate-500 text-xs font-semibold"
                                                        x-text="parseFloat(item[1]).toFixed(1) + '%'"></div>
                                                </div>
                                            </div>
                                        </template>
                                    </div>
                                    ...
                                    <div class="pie-canvas-wrap flex-shrink-0 flex items-center justify-center">
                                        <canvas id="disciplineMarketShareChart" width="400"
                                            height="350"></canvas>
                                    </div>
                            ...
                            <div class="pie-mini-legend">
                                ...
                                return entries
                                    ...
                                    <div class="pie-mini-legend-item">
                                        ...
                                        <div>
                                            <div class="pie-mini-legend-name" x-text="formatDisciplineName(item[0])">
                                            </div>
                                            <div class="pie-mini-legend-pct"
                                                x-text="parseFloat(item[1]).toFixed(1) + '%'"></div>
                                        </div>
                                    </div>
                                </template>
                            ...
                            <template x-teleport="body">
                                <div x-show="pieModalOpen" x-transition:enter="transition ease-out duration-200"
                                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                                    x-transition:leave="transition ease-in duration-150"
                                    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                                    class="fixed inset-0 z-[9999] bg-black/60 flex items-end sm:items-center justify-center p-0 sm:p-4"
                                    style="display:none;">
                                    <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-[80vw] flex flex-col overflow-hidden"
                                        style="height:92vh; max-height:92vh;" @click.stop>
                                        <!-- Header -->
                                        <div
                                            class="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-slate-200 flex-shrink-0">
                                            <div>
                                                <h2 class="text-base sm:text-xl font-bold text-slate-800">Distribution
                                                    of Enrollees — Expanded View</h2>
                                                <p class="text-sm text-slate-500 mt-0.5"><span
                                                        x-text="selectedEnrollmentYear"></span> &bull; <span
                                                        x-text="selectedEnrollmentProvince"></span></p>
                                            </div>
                                        ...
                                        <div class="flex-1 min-h-0 overflow-hidden flex flex-col sm:flex-row">
                                            ...
                                            <div class="flex flex-col w-full h-full sm:hidden">
                                                <div
                                                    class="flex-shrink-0 flex justify-center items-center py-3 border-b border-slate-100">
                                                    <canvas id="disciplineMarketShareChartModalMobile" width="300"
                                                        height="300"
                                                        style="width:300px;height:300px;display:block;"></canvas>
                                                </div>
                                                <div class="flex-1 min-h-0 overflow-y-auto px-4 py-3 custom-scrollbar">
                                                    <p
                                                        class="text-xs font-bold text-slate-400 uppercase tracking-wide mb-3">
                                                        All Disciplines</p>
                                                    <div
                                                        style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem 0.75rem;">
                                                        <template
                                                            x-for="(item) in (() => {
                                                            ...
                                                            <div class="flex items-start gap-1.5 min-w-0">
                                                                ...
                                                                <div class="min-w-0">
                                                                    <p class="text-xs font-medium text-slate-700 leading-tight"
                                                                        x-text="formatDisciplineName(item[0])"></p>
                                                                    <p class="text-xs text-slate-400"
                                                                        x-text="parseFloat(item[1]).toFixed(1)+'%'">
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </template>
                                            ...
                                            <div
                                                class="hidden sm:flex flex-1 items-center justify-center gap-6 p-8 overflow-y-auto custom-scrollbar">
                                                        ...
                                                        <div class="flex items-start gap-2 min-w-0">
                                                            ...
                                                            <div class="min-w-0">
                                                                <p class="text-xs font-semibold text-slate-800 leading-snug"
                                                                    x-text="formatDisciplineName(item[0])"></p>
                                                                <p class="text-xs text-slate-400"
                                                                    x-text="parseFloat(item[1]).toFixed(1)+'%'"></p>
                                                            </div>
                                                        </div>
                                                    </template>
                                                </div>
                                                <div class="flex-shrink-0 flex items-center justify-center"
                                                    ...
                                                    <canvas id="disciplineMarketShareChartModal" width="420"
                                                        height="420"></canvas>
                                                </div>
                ...
                <div class="relative flex items-center gap-4 my-2">
                    ...
                    <span
                        class="text-xs font-bold text-slate-400 uppercase tracking-widest px-2 whitespace-nowrap">Data
                        Charts</span>
                    <div class="flex-1 h-px bg-slate-300"></div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden mt-6" x-data="{ enrollmentTrendExpanded: false }">
                    ...
                    <button
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="text-left">
                                ...
                                <p class="text-xs text-slate-400">
                                    ...
                                    <span x-show="!enrollmentTrendExpanded" class="text-slate-500 ml-2">• Click to
                                        expand</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <svg class="w-5 h-5 text-slate-400 transition-transform duration-300"
                            :class="{ 'rotate-180': enrollmentTrendExpanded }" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7">
                            </path>
                        </svg>
                    ...
                    <!-- Collapsible Content -->
                    <div x-show="enrollmentTrendExpanded" x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                        x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
                        x-transition:leave-end="opacity-0">
                        ...
                        <!-- Filters Bar -->
                        <div
                            class="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200 px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-2 sm:gap-3">
                            <!-- Left: dynamic label -->
                            <p class="text-xs text-slate-500 w-full sm:w-auto">
                                Student enrollment public and private - <span x-text="selectedTrendYear"
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                <div
                                    ...
                                    <span class="text-xs text-slate-500 font-medium hidden sm:inline">Province:</span>
                                    <select x-model="selectedTrendProvince"
                                ...
                                <div
                                    ...
                                    <span class="text-xs text-slate-500 font-medium hidden sm:inline">Year:</span>
                                    <select x-model="selectedTrendYear" @change="updateTrendChart()"
                                        class="text-sm font-bold text-slate-800 bg-transparent focus:outline-none cursor-pointer">
                                ...
                                <!-- Expand Button -->
                                <button @click="enrollmentTrendModalOpen = true"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4">
                            <div
                                class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-3 border border-blue-200">
                            ...
                            <div
                                class="bg-gradient-to-br from-sky-50 to-indigo-50 rounded-lg p-3 border border-sky-200">
                        ...
                        <div class="enrollment-trend-mobile px-4 pb-6 pt-2">
                            <div x-show="loadingEnrollmentTrend" class="flex items-center justify-center py-10">
                                <div
                                    class="animate-spin rounded-full h-8 w-8 border-2 border-slate-200 border-t-slate-500">
                                </div>
                            </div>
                            <div x-show="!loadingEnrollmentTrend && trendTableData.length === 0"
                                class="text-center py-8 text-slate-400 text-sm italic">
                        ...
                        <div class="enrollment-trend-desktop p-6">
                            <div class="relative w-full border-2 border-slate-200 rounded-lg p-3 bg-white"
                                ...
                                <div x-show="loadingEnrollmentTrend"
                                    class="absolute inset-0 bg-white/75 flex items-center justify-center z-10 rounded-lg backdrop-blur-sm">
                                    <div
                                        class="animate-spin rounded-full h-10 w-10 border-2 border-slate-200 border-t-slate-500">
                                    </div>
                                </div>
                        ...
                        <template x-teleport="body">
                            <div x-show="enrollmentTrendModalOpen"
                                x-transition:enter="transition ease-out duration-200"
                                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                                x-transition:leave="transition ease-in duration-150"
                                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                                class="fixed inset-0 z-[9999] bg-black/60 flex items-center justify-center p-4"
                                @keydown.escape.window="enrollmentTrendModalOpen = false" style="display:none;">
                                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-[95vw] flex flex-col overflow-hidden"
                                    style="height:92vh;" @click.stop>
                                    <!-- Modal Header -->
                                    <div
                                        class="flex flex-wrap items-start justify-between gap-3 px-4 sm:px-6 py-4 border-b border-slate-200 flex-shrink-0">
                                        <div>
                                            ...
                                            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">
                                                Public vs. Private &bull; <span x-text="selectedTrendProvince"></span>
                                                &bull; <span x-text="selectedTrendYear"></span>
                                            </p>
                                        </div>
                                        <div class="flex items-center gap-2 sm:gap-3">
                                            ...
                                            <div class="trend-modal-legend flex items-center gap-4 mr-2 sm:mr-4">
                                                <div class="flex items-center gap-2">
                                                    <div class="w-5 h-3 bg-blue-600 rounded"></div><span
                                                        class="text-xs text-slate-600 font-medium">Public</span>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <div class="w-5 h-3 bg-sky-400 rounded"></div><span
                                                        class="text-xs text-slate-600 font-medium">Private</span>
                                                </div>
                                            </div>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 px-6 pt-4 flex-shrink-0">
                                        <div
                                            class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-3 border border-blue-200">
                                            <p class="text-xs font-semibold text-blue-700 mb-1">Total Public Schools
                                            </p>
                                            <p class="text-2xl font-bold text-blue-900"
                                                x-text="enrollmentTrendTotals.public ? enrollmentTrendTotals.public.toLocaleString() : '—'">
                                            </p>
                                        </div>
                                        <div
                                            class="bg-gradient-to-br from-sky-50 to-indigo-50 rounded-lg p-3 border border-sky-200">
                                            <p class="text-xs font-semibold text-sky-700 mb-1">Total Private Schools
                                            </p>
                                            <p class="text-2xl font-bold text-sky-900"
                                                x-text="enrollmentTrendTotals.private ? enrollmentTrendTotals.private.toLocaleString() : '—'">
                                            </p>
                                        </div>
                                    </div>
                                    <!-- Chart -->
                ...
                <div class="mt-6">
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden" x-data="{ disciplineEnrollmentExpanded: false, disciplineEnrollmentInitialized: false }">
                        ...
                        <button
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="text-left">
                                    ...
                                    <p class="text-xs text-slate-400">
                                        ...
                                        <span x-show="!disciplineEnrollmentExpanded" class="text-slate-500 ml-2">•
                            ...
                            <!-- Expand/Collapse Icon -->
                            <svg class="w-5 h-5 text-slate-400 transition-transform duration-300"
                                :class="{ 'rotate-180': disciplineEnrollmentExpanded }" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <!-- Collapsible Content -->
                        <div x-show="disciplineEnrollmentExpanded"
                            ...
                            <div class="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200 p-4">
                                <div class="flex flex-wrap items-center justify-between mb-3 gap-2">
                                    <div class="flex items-center gap-3">
                                        <p class="text-xs text-slate-500">
                                            Student enrollment by discipline - <span x-text="selectedEnrollmentYear"
                                                class="font-semibold text-blue-600"></span>
                                            <span x-show="selectedEnrollmentProvince !== 'Davao Region'"
                                                class="text-slate-400"> • </span>
                                            <span x-show="selectedEnrollmentProvince !== 'Davao Region'"
                                                x-text="selectedEnrollmentProvince"
                                    ...
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        <div
                                            ...
                                            <span
                                                class="text-xs text-slate-500 font-medium hidden sm:inline">Province:</span>
                                            <select x-model="selectedEnrollmentProvince"
                                                @change="loadEnrollmentYearsForProvince(selectedEnrollmentProvince).then(() => loadEnrollmentData())"
                                                class="text-sm font-bold text-slate-800 bg-transparent focus:outline-none cursor-pointer">
                                                <template x-for="province in availableEnrollmentProvinces"
                                                    :key="province">
                                                    <option :value="province" x-text="province"></option>
                                                </template>
                                            </select>
                                        ...
                                        <!-- Year Selector -->
                                        <div x-show="availableEnrollmentYears.length > 0"
                                            class="flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl px-3 py-1.5 shadow-sm">
                                            <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <span
                                                class="text-xs text-slate-500 font-medium hidden sm:inline">Year:</span>
                                            <select x-model="selectedEnrollmentYear" @change="loadEnrollmentData()"
                                                class="text-sm font-bold text-slate-800 bg-transparent focus:outline-none cursor-pointer">
                                                <template x-for="year in availableEnrollmentYears"
                                                    :key="year">
                                                    <option :value="year" x-text="year"></option>
                                                </template>
                                            </select>
                                        ...
                                        <!-- Expand Button -->
                                        <button @click="disciplineEnrollmentModalOpen = true"
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div
                                        ...
                                        <p class="text-2xl font-bold text-blue-900" x-text="enrollmentData.length">
                                        </p>
                                    </div>
                                    <div
                                        ...
                                        <p class="text-2xl font-bold text-purple-900"
                                            x-text="getTotalEnrollment().toLocaleString()"></p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-6">
                                ...
                                <div x-show="enrollmentData.length === 0"
                                    ...
                                    <svg class="w-20 h-20 text-slate-300 mb-4" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253">
                                        </path>
                                    ...
                                    <p class="text-sm text-slate-500">No disciplines found for <span
                                            x-text="selectedEnrollmentYear"></span></p>
                                </div>
                                ...
                                <!-- ══════════════════════════════════════════════════════ -->
                                <div x-show="enrollmentData.length > 0"
                                    class="discipline-enrollment-mobile space-y-0">
                                    <div x-show="loadingDisciplineEnrollment"
                                    ...
                                    <template x-if="!loadingDisciplineEnrollment && enrollmentData.length > 0">
                                        <div>
                                            <template
                                                x-for="(row, rowIndex) in [...enrollmentData].sort((a,b) => b.count - a.count)"
                                                :key="row.discipline">
                                                <div class="py-2 border-b border-slate-100 last:border-0 trend-row-animate"
                                                    :style="`animation-delay: ${rowIndex * 40}ms`">
                                                    <div class="flex items-center justify-between mb-1">
                                                        <span
                                                            class="text-xs font-semibold text-slate-700 truncate pr-2"
                                                            x-text="row.discipline" style="max-width: 60%;"></span>
                            ...
                            <template x-teleport="body">
                                <div x-show="disciplineEnrollmentModalOpen"
                                    x-transition:enter="transition ease-out duration-200"
                                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                                    x-transition:leave="transition ease-in duration-150"
                                    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                                    ...
                                    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-[95vw] flex flex-col overflow-hidden"
                                        ...
                                        <div
                                            class="flex flex-wrap items-start justify-between gap-3 px-4 sm:px-6 py-4 border-b border-slate-200 flex-shrink-0">
                                            <div>
                                                <h2 class="text-sm sm:text-xl font-bold text-slate-800">Enrollment by
                                                    Discipline — Expanded View</h2>
                                                <p class="text-xs sm:text-sm text-slate-500 mt-0.5">
                                                    <span x-text="enrollmentData.length"></span> disciplines &bull;
                                                    <span x-text="selectedEnrollmentYear"></span>
                                                    <span x-show="selectedEnrollmentProvince !== 'Davao Region'"
                                                        x-text="' • ' + selectedEnrollmentProvince"></span>
                                                </p>
                                            </div>
                                            <div class="flex items-center gap-2 sm:gap-3">
                                                <div class="hidden sm:flex items-center gap-2 mr-2">
                                                    <div class="w-20 h-3 rounded"
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 px-6 pt-4 flex-shrink-0">
                                            <div
                                                class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-3 border border-blue-200">
                                                <p class="text-xs font-semibold text-blue-700 mb-1">Total Disciplines
                                                </p>
                                                <p class="text-2xl font-bold text-blue-900"
                                                    x-text="enrollmentData.length"></p>
                                            </div>
                                            <div
                                                class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-3 border border-purple-200">
                                                <p class="text-xs font-semibold text-purple-700 mb-1">Total Enrolled
                                                </p>
                                                <p class="text-2xl font-bold text-purple-900"
                                                    x-text="getTotalEnrollment().toLocaleString()"></p>
                                            </div>
                                        ...
                                        <!-- Chart -->
                                        <div
                                            class="flex-1 min-h-0 overflow-y-auto overflow-x-hidden custom-scrollbar px-6 pb-6 pt-4">
                                            <div
                                                :style="`height: ${Math.max(enrollmentData.length * 52, 300)}px; min-height: 200px; width: 100%;`">
                                                <canvas id="disciplineEnrollmentChartModal"
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden mt-6" x-data="{ licensureExpanded: false, licensureInitialized: false }">
                    ...
                    <button
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="text-left">
                                ...
                                <p class="text-xs text-slate-400">
                                    ...
                                    <span x-show="!licensureExpanded" class="text-slate-500 ml-2">• Click to
                                        expand</span>
                                </p>
                            </div>
                        </div>
                        <!-- Expand/Collapse Icon -->
                        <svg class="w-5 h-5 text-slate-400 transition-transform duration-300"
                            :class="{ 'rotate-180': licensureExpanded }" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7">
                            </path>
                        </svg>
                    ...
                    <!-- Collapsible Content -->
                    <div x-show="licensureExpanded" x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                        x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
                        x-transition:leave-end="opacity-0">
                        ...
                        <div class="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200 p-4">
                            <div class="flex flex-wrap items-center justify-between gap-4">
                                ...
                                <div class="flex items-center gap-3">
                                    <p class="text-xs text-slate-500">
                                        Performance in regulated professions - <span x-text="selectedYear"
                                            class="font-semibold text-emerald-600"></span>
                                        <span x-show="selectedSector !== 'all'"
                                            class="ml-2 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-xs font-semibold">
                                ...
                                <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <select x-model="selectedSector" @change="updateChart()"
                            ...
                            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div
                                    class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg p-3 border border-emerald-200">
                                    <p class="text-xs font-semibold text-emerald-700 mb-1">Total Professions</p>
                                    <p class="text-2xl font-bold text-emerald-900" x-text="getFilteredData().length">
                                    </p>
                                </div>
                                <div
                                    class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg p-3 border border-amber-200">
                        ...
                        <div class="p-6">
                            ...
                            <div x-show="getFilteredData().length === 0"
                                ...
                                <p class="text-sm text-slate-500">No professions found for <span
                                        x-text="selectedSector"></span> in <span x-text="selectedYear"></span></p>
                            </div>
                            ...
                            <div x-show="getFilteredData().length > 0" class="licensure-mobile space-y-0">
                                ...
                                <template x-if="!loadingLicensure && getFilteredData().length > 0">
                                    <div>
                                        <template
                                            x-for="(item, rowIndex) in [...getFilteredData()].sort((a,b) => b.passing_rate - a.passing_rate)"
                                            :key="item.profession">
                                            <div class="py-2.5 border-b border-slate-100 last:border-0 trend-row-animate"
                                                ...
                                                <div class="flex items-start justify-between gap-2 mb-1">
                                                    <span class="text-xs font-semibold text-slate-800 leading-snug"
                                                        x-text="item.profession" style="max-width: 65%;"></span>
                                                    ...
                                                    <div class="h-full trend-bar-animate"
                                                        :style="(() => {
                                                            ...
                                                            if (selectedSector !== 'all' && hex) {
                                                                ...
                                                                sR = Math.round(br * 0.5);
                                                                sG = Math.round(bg * 0.5);
                                                                sB = Math.round(bb * 0.5);
                                                                eR = Math.min(255, Math.round(br + (255 - br) *
                                                                    0.6));
                                                                eG = Math.min(255, Math.round(bg + (255 - bg) *
                                                                    0.6));
                                                                eB = Math.min(255, Math.round(bb + (255 - bb) *
                                                                    0.6));
                                                            } else {
                                                                sR = 30;
                                                                sG = 41;
                        ...
                        <template x-teleport="body">
                            <div x-show="expanded" x-transition:enter="transition ease-out duration-200"
                                ...
                                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                                class="fixed inset-0 z-[9999] bg-black/60 flex items-center justify-center p-4"
                                @keydown.escape.window="expanded = false" style="display:none;">
                                ...
                                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-[95vw] flex flex-col overflow-hidden"
                                    ...
                                    <div
                                        class="flex flex-wrap items-start justify-between gap-3 px-4 sm:px-6 py-4 border-b border-slate-200 flex-shrink-0">
                                        <div>
                                            <h2 class="text-sm sm:text-xl font-bold text-slate-800">Licensure Passing
                                                Rates — Expanded View</h2>
                                            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">
                                                <span x-show="selectedSector !== 'all'"
                                                    class="mr-1 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-xs font-semibold"
                                                    x-text="selectedSector"></span>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 px-6 pt-4 flex-shrink-0">
                                        <div
                                            class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg p-3 border border-emerald-200">
                                            <p class="text-xs font-semibold text-emerald-700 mb-1">Total Professions
                                            </p>
                                            <p class="text-2xl font-bold text-emerald-900"
                                                x-text="getFilteredData().length"></p>
                                        </div>
                                        <div
                                            class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg p-3 border border-amber-200">
                                            <p class="text-xs font-semibold text-amber-700 mb-1">Highest Passing Rate
                                            </p>
                                            <p class="text-2xl font-bold text-amber-900"
                                                x-text="getHighestRate() + '%'"></p>
                                        </div>
                                    </div>
                                    ...
                                    <div
                                        class="flex-1 min-h-0 overflow-y-auto overflow-x-hidden custom-scrollbar px-6 pb-6 pt-4">
                                        <div
                                            :style="`height: ${getExpandedChartHeight()}px; min-height: 200px; width: 100%;`">
