                    ...
                    <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                <h3 class="text-base font-bold text-slate-800">Currently Published</h3>
                            </div>

                            <div x-show="loadingPublished" class="flex justify-center py-4">
                                <div class="animate-spin rounded-full h-5 w-5 border-b-2 border-green-600"></div>
                            </div>

                            <div x-show="!loadingPublished">
                                <!-- Has published content -->
                                <template x-if="publishedExists">
                                    <div>
                                        <p class="text-xs text-slate-500 mb-2">
                                            Last published: <span x-text="publishedUpdatedAt"></span>
                                        </p>
                                        <div class="bg-green-50 border border-green-200 rounded-lg p-3 max-h-48 overflow-y-auto">
                                            <p class="text-xs text-slate-700 line-clamp-6" x-html="publishedText"></p>
                                        </div>
                                        <button
                                ...
                                <template x-if="!publishedExists">
                                    <div class="text-center py-6 text-slate-400">
                                        ...
                                        <p class="text-xs">No published content yet for this selection.</p>
                                    </div>
                                </template>
                            </div>
                        </div>

                        <!-- Archived Analyses Card -->
                        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                            <div class="flex items-center gap-2 mb-4">
                                <svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path>
                                </svg>
                                <h3 class="text-base font-bold text-slate-800">Past Published Versions</h3>
                            </div>
                            <p class="text-xs text-slate-500 mb-4">Copy text from previous published analyses to save time.</p>

                            <div x-show="loadingArchives" class="flex justify-center py-4">
                                <div class="animate-spin rounded-full h-5 w-5 border-b-2 border-indigo-600"></div>
                            </div>

                            <div x-show="!loadingArchives" class="space-y-3 max-h-72 overflow-y-auto">
                                <template x-for="archive in archivedAnalyses" :key="archive.id">
                                    <div class="border border-slate-200 rounded-lg p-3 hover:border-indigo-300 hover:bg-indigo-50 transition cursor-pointer group"
                                        ...
                                        <p class="text-xs text-slate-600 line-clamp-2" x-html="archive.analysis_text"></p>
                                    </div>
                                </template>

                                <div x-show="archivedAnalyses.length === 0" class="text-center py-6 text-slate-400">
                                    <p class="text-xs">No published archives for this selection.</p>
                                </div>
                            </div>
                        </div>
            ...
            return {
                ...
                initQuillEditor() {
                    ...
                    if (this.analysisText) {
                        this.quill.root.innerHTML = this.analysisText;
                    }
                },
