...
<head>
    ...
    <style>
        ...
        .plus-btn { transition: all 0.2s ease; }
        .plus-btn:hover { transform: scale(1.08); box-shadow: 0 0 0 4px rgba(99,102,241,0.25); }
        .modal-backdrop { backdrop-filter: blur(4px); }
        .add-zone { border: 2px dashed #c7d2fe; transition: all 0.2s; }
        .add-zone:hover { border-color: #6366f1; background: #eef2ff; }
        .program-row .admin-actions { opacity: 0; transition: opacity 0.15s ease; }
        .program-row:hover .admin-actions { opacity: 1; }
        .program-row:hover .program-name { color: var(--program-color); }
        ...
        .qual-content p { margin: 0; padding: 0; }
        .qual-content p+p { margin-top: 0.25rem; }
        .qual-content p:empty { display: none; }
        .qual-content ul { list-style: disc; padding-left: 1.25rem; }
        .qual-content ol { list-style: decimal; padding-left: 1.25rem; }
        ...
        .story-card-admin { position: absolute; top: 5px; right: 5px; display: flex; gap: 3px; opacity: 0; transition: opacity 0.2s ease; z-index: 10; }
        .story-card-slide:hover .story-card-admin { opacity: 1; }
        ...
        .story-add-slot { flex: 0 0 calc(20% - 10px); min-height: 175px; border: 2px dashed #c7d2fe; border-radius: 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 7px; color: #818cf8; font-size: 0.75rem; font-weight: 600; cursor: pointer; background: white; transition: all 0.2s ease; }
        .story-add-slot:hover { border-color: #6366f1; background: #eef2ff; color: #6366f1; transform: translateY(-4px); }
...
<body class="bg-slate-100 min-h-screen" x-data="{ ...adminPage(), previewOpen: false, previewSrc: '' }">
    ...
    {{-- ===== ADMIN TOP BAR ===== --}}
    <header class="bg-white h-16 shrink-0 border-b border-slate-200 flex items-center justify-between px-8 shadow-sm sticky top-0 z-50">
    ...
    <div x-show="previewOpen"
        ...
        class="fixed top-0 right-0 h-screen z-[70] flex flex-col shadow-2xl"
        x-init="$el.style.left = ($refs.mainContent?.offsetLeft ?? 256) + 'px'"
        x-cloak>
        ...
        <div class="flex items-center justify-between px-5 py-3 shrink-0 bg-white border-b border-slate-200">
            ...
            <div class="flex items-center gap-2">
                <button @click="$refs.previewFrame.src = $refs.previewFrame.src"
    ...
    @php
        ...
        foreach ($programs as $p) {
            ...
            if ($p->acronym) $programColorMap[$p->acronym] = $p->color;
        }
    @endphp

    {{-- ===== CAROUSEL SECTION ===== --}}
    @php
        $slidesJson = $carouselSlides->map(fn($s) => [
    ...
    <div id="carousel-section" class="relative w-full shrink-0 overflow-hidden" style="height: calc(100vh - 64px);"
        x-data="carouselSection({{ $slidesJson }})"
        ...
        <template x-for="(slide, index) in slides" :key="index">
            <div x-show="currentSlide === index"
                ...
                <div class="absolute inset-0">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-b from-slate-900/85 via-slate-900/70 to-slate-900/50"></div>
                </div>
                <div class="relative z-10 h-full flex items-center justify-center px-4">
                    <div class="flex flex-col items-center justify-center text-center text-white max-w-5xl h-full py-20">
                        <div class="flex-grow flex flex-col justify-center mb-8">
                            <h1 class="text-4xl md:text-5xl lg:text-6xl font-extrabold mb-4 drop-shadow-2xl leading-tight line-clamp-2" x-text="slide.title"></h1>
                            <p class="text-lg md:text-xl lg:text-2xl text-slate-50 drop-shadow-lg max-w-4xl mx-auto leading-relaxed font-light line-clamp-3" x-text="slide.excerpt"></p>
                        </div>
                        <div class="flex-shrink-0 pb-12">
                            <div class="inline-flex items-center gap-3 px-10 py-5 bg-white text-slate-900 font-bold text-lg rounded-xl opacity-80 cursor-default">
                ...
                <div class="absolute top-6 right-6 z-30 flex gap-2">
                    <button @click="$dispatch('open-modal', { type: 'edit-slide', id: slide.id, data: { ...slide, image_url: slide.image, image: null } })"
        ...
        <button @click="prevSlide()" class="absolute left-6 top-1/2 -translate-y-1/2 z-20 w-14 h-14 bg-white/25 hover:bg-white/40 backdrop-blur-md rounded-full flex items-center justify-center transition-all duration-300 border border-white/30">
            <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M15 19l-7-7 7-7" />
            </svg>
        </button>
        <button @click="nextSlide()" class="absolute right-6 top-1/2 -translate-y-1/2 z-20 w-14 h-14 bg-white/25 hover:bg-white/40 backdrop-blur-md rounded-full flex items-center justify-center transition-all duration-300 border border-white/30">
            <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="absolute bottom-24 left-1/2 -translate-x-1/2 z-20 flex items-center gap-4">
            <template x-for="(slide, index) in slides" :key="index">
                <button @click="goToSlide(index)" class="transition-all duration-300" :class="currentSlide === index ? 'w-16 h-4' : 'w-4 h-4'">
                    <div class="w-full h-full rounded-full backdrop-blur-md border-2" :class="currentSlide === index ? 'bg-white border-white' : 'bg-white/40 border-white/60'"></div>
                </button>
            </template>
        </div>
    ...
    <div id="programs-section" class="w-full py-16" style="padding-left: 7.5rem; padding-right: 7.5rem;">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-slate-300 text-sm md:text-base">Click on any program below to view details and eligibility</p>
                    </div>
                </div>
        ...
        <div id="programs-ajax-container" x-data="{ openId: null }" class="bg-white rounded-2xl shadow-2xl border border-slate-200 divide-y divide-slate-200 overflow-hidden">
            ...
            @foreach ($programs as $program)
                @php
                    $colorMap = [
                        ...
                        'yellow' => ['50'=>'#fefce8','100'=>'#fef9c3','200'=>'#fef08a','400'=>'#facc15','500'=>'#eab308','600'=>'#ca8a04'],
                        'green'  => ['50'=>'#f0fdf4','100'=>'#dcfce7','200'=>'#bbf7d0','400'=>'#4ade80','500'=>'#22c55e','600'=>'#16a34a'],
                        ...
                        'indigo' => ['50'=>'#eef2ff','100'=>'#e0e7ff','200'=>'#c7d2fe','400'=>'#818cf8','500'=>'#6366f1','600'=>'#4f46e5'],
                        'violet' => ['50'=>'#f5f3ff','100'=>'#ede9fe','200'=>'#ddd6fe','400'=>'#a78bfa','500'=>'#8b5cf6','600'=>'#7c3aed'],
                        'purple' => ['50'=>'#faf5ff','100'=>'#f3e8ff','200'=>'#e9d5ff','400'=>'#c084fc','500'=>'#a855f7','600'=>'#9333ea'],
                        ...
                        'teal'   => ['50'=>'#f0fdfa','100'=>'#ccfbf1','200'=>'#99f6e4','400'=>'#2dd4bf','500'=>'#14b8a6','600'=>'#0d9488'],
                        'sky'    => ['50'=>'#f0f9ff','100'=>'#e0f2fe','200'=>'#bae6fd','400'=>'#38bdf8','500'=>'#0ea5e9','600'=>'#0284c7'],
                        'lime'   => ['50'=>'#f7fee7','100'=>'#ecfccb','200'=>'#d9f99d','400'=>'#a3e635','500'=>'#84cc16','600'=>'#65a30d'],
                    ];
                    ...
                    $testimonialsJson = ($program->testimonials ?? collect())->map(fn($t) => [
                        'id'          => $t->id,
                ...
                <div id="program-card-{{ $program->id }}" x-data="{ id: {{ $program->id }} }">
                    <div class="relative program-row" style="--program-color: {{ $c['600'] }}">
                        <button @click="openId = (openId === id) ? null : id"
                            ...
                            <div class="flex items-center gap-5">
                                <div class="w-16 h-16 md:w-20 md:h-20 rounded-2xl flex-shrink-0 flex items-center justify-center shadow-md transition-all duration-300 border-2"
                                    :style="openId === id ? 'background:white; border-color:{{ $c['400'] }}' : 'background:{{ $c['50'] }}; border-color:transparent'">
                                    <img src="{{ asset($program->logo_path) }}" alt="{{ $program->name }} Logo" class="w-10 h-10 md:w-14 md:h-14 object-contain">
                                </div>
                                <div>
                                    <h3 class="text-xl md:text-2xl font-bold text-slate-900">
                                        <span class="program-name transition-colors duration-200">{{ $program->name }}</span>
                                    </h3>
                                    <p class="text-sm md:text-base text-slate-500 mt-1">{{ $program->subtitle }}</p>
                                    <div class="flex items-center gap-2 mt-1.5">
                                        <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full {{ $program->is_published ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">
                                            <span class="w-1.5 h-1.5 rounded-full {{ $program->is_published ? 'bg-emerald-500' : 'bg-amber-500' }}"></span>
                                            {{ $program->is_published ? 'Published' : 'Draft' }}
                                        </span>
                                        @if ($program->is_published && $program->has_draft_changes)
                                            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-orange-100 text-orange-600 border border-orange-200">
                            ...
                            <div class="flex items-center gap-2 flex-shrink-0 ml-4">
                                <span class="text-xs md:text-sm font-semibold hidden sm:block" :style="openId === id ? 'color:{{ $c['600'] }}' : 'color:#94a3b8'">
                                ...
                                <div class="w-9 h-9 rounded-full flex items-center justify-center transition-colors duration-200"
                                    ...
                                    <svg class="w-5 h-5 transition-transform duration-300 chevron-icon" :class="openId === id ? 'open text-white' : 'text-slate-500'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {{-- Edit / Delete --}}
                        <div class="admin-actions absolute bottom-2 right-6 md:right-10 flex items-center gap-1.5 z-10" @click.stop>
                            <button @click="$dispatch('open-modal', { type: 'edit-program', id: {{ $program->id }}, data: {{ $editProgramData }} })"
                            ...
                            <button @click="$dispatch('open-modal', { type: 'delete-program', id: {{ $program->id }} })"
                    ...
                    <div x-show="openId === id" x-collapse x-cloak>
                        <div id="program-body-{{ $program->id }}" class="border-t border-slate-200 p-6 md:p-10" style="background: linear-gradient(to bottom right, #f8fafc, {{ $c['50'] }}33)">
                            <div class="grid lg:grid-cols-3 gap-8">

                                <div class="lg:col-span-2 space-y-6">

                                    {{-- Description --}}
                                    <div class="rounded-xl p-6 relative group/card border" style="background:{{ $c['50'] }}; border-color:{{ $c['200'] }}">
                                        <h4 class="font-bold text-slate-800 mb-3 flex items-center gap-2 text-lg">
                                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="color:{{ $c['600'] }}">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                            Program Details
                                        </h4>
                                        {{-- FIX #1: Sanitize HTML output — allow only safe formatting tags --}}
                                        <div class="text-slate-700 leading-relaxed prose prose-sm max-w-none">{!! strip_tags($program->description, '<b><strong><em><i><u><ul><ol><li><p><br><a><span><h1><h2><h3><h4>') !!}</div>
                                        <span class="absolute top-3 right-3 flex gap-1 opacity-0 group-hover/card:opacity-100 transition-opacity">
                                            <button @click="$dispatch('open-modal', { type: 'edit-description', id: {{ $program->id }}, data: { description: {{ Js::from($program->description) }} } })"
                                            ...
                                            <button @click="$dispatch('open-modal', { type: 'delete-item', id: {{ $program->id }}, programId: {{ $program->id }}, endpoint: '/admin/programs/{{ $program->id }}/description' })"
                                    ...
                                    <div class="grid md:grid-cols-2 gap-6">
                                        @foreach ($groupedQuals as $type => $items)
                                            <div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm group/catcard">
                                                <div class="flex items-center justify-between mb-4">
                                                    ...
                                                    <button @click="$dispatch('open-modal', { type: 'delete-item', id: {{ $items->first()->id }}, programId: {{ $program->id }}, endpoint: '/admin/qualifications/type/{{ urlencode($type) }}/program/{{ $program->id }}' })"
                                                ...
                                                <ul class="space-y-3 text-slate-700 text-sm text-justify">
                                                    @foreach ($items as $q)
                                                        <li class="flex items-start gap-2 group/item">
                                                            <span class="font-bold mt-0.5" style="color:{{ $c['500'] }}">•</span>
                                                            {{-- FIX #1: Sanitize HTML output — allow only safe formatting tags --}}
                                                            <span class="flex-1 qual-content">{!! strip_tags($q->content, '<b><strong><em><i><u><ul><ol><li><p><br><a><span>') !!}</span>
                                                            <span class="flex gap-1 opacity-0 group-hover/item:opacity-100 transition flex-shrink-0">
                                                                <button @click="$dispatch('open-modal', { type: 'edit-qualification', id: {{ $q->id }}, programId: {{ $program->id }}, data: { type: '{{ e($q->type) }}', content: {{ Js::from($q->content) }} } })"
                                                                    class="w-5 h-5 bg-indigo-100 hover:bg-indigo-500 text-indigo-500 hover:text-white rounded flex items-center justify-center transition">
                                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                                    </svg>
                                                                </button>
                                                                <button @click="$dispatch('open-modal', { type: 'delete-item', id: {{ $q->id }}, programId: {{ $program->id }}, endpoint: '/admin/qualifications/{{ $q->id }}' })"
                                                                    class="w-5 h-5 bg-red-100 hover:bg-red-500 text-red-500 hover:text-white rounded flex items-center justify-center transition">
                                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                                                    </svg>
                                        ...
                                        <div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                                            <div class="flex items-center justify-between mb-4">
                                                ...
                                                <button @click="$dispatch('open-modal', { type: 'add-qualification', programId: {{ $program->id }}, data: {} })"
                                            ...
                                            <p class="text-xs text-slate-400">Add a qualification, requirement, beneficiary, service, or objective.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="text-white rounded-xl p-6" style="background:{{ $c['600'] }}">
                                        <div class="flex items-center justify-between mb-4">
                                            ...
                                            <button @click="$dispatch('open-modal', { type: 'add-step', programId: {{ $program->id }} })"
                                        ...
                                        <ol class="space-y-3 text-sm">
                                            @foreach ($program->howToApply as $step)
                                                <li class="flex items-start gap-3 group/item">
                                                    <span class="font-bold flex-shrink-0" style="color:{{ $c['200'] }}">{{ $loop->iteration }}.</span>
                                                    <span class="flex-1">
                                                        {{-- FIX #1: Sanitize step content before rendering --}}
                                                        {!! strip_tags(trim(preg_replace('/<\/?p[^>]*>/', ' ', $step->content)), '<b><strong><em><i><u><br><span>') !!}
                                                        @if ($step->link)
                                                            <a href="{{ $step->link }}" target="_blank" class="inline-flex items-center gap-1 underline font-semibold hover:opacity-80 transition ml-1" style="color:white;">
                                                    ...
                                                    <span class="flex gap-1 opacity-0 group-hover/item:opacity-100 transition flex-shrink-0">
                                                        <button @click="$dispatch('open-modal', { type: 'edit-step', id: {{ $step->id }}, programId: {{ $program->id }}, data: { content: {{ Js::from($step->content) }}, link: {{ Js::from($step->link) }} } })"
                                                            class="w-5 h-5 bg-white/20 hover:bg-white text-white rounded flex items-center justify-center transition"
                                                            onmouseover="this.style.color='{{ $c['600'] }}'" onmouseout="this.style.color='white'">
                                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                            </svg>
                                                        </button>
                                                        <button @click="$dispatch('open-modal', { type: 'delete-item', id: {{ $step->id }}, programId: {{ $program->id }}, endpoint: '/admin/steps/{{ $step->id }}' })"
                                    ...
                                    <div x-data="storiesCarousel('{{ $programCarouselId }}', '{{ $c['600'] }}')" x-init="init()" id="{{ $programCarouselId }}-wrapper">
                                        <div class="flex items-center mb-3">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <h4 class="font-bold text-slate-800">{{ $program->name }} Success Stories</h4>
                                            </div>
                                        </div>
                                                        ...
                                                        <div class="story-card-slide">
                                                            <div class="story-card-img">
                                                                ...
                                                                <div class="story-card-admin">
                                                                    <button @click.stop="$dispatch('open-modal', { type: 'edit-story', id: {{ $story->id }}, programId: {{ $program->id }}, data: { title: {{ Js::from($story->title) }}, link: {{ Js::from($story->link) }} } })"
                                                                        class="w-6 h-6 bg-indigo-600 text-white rounded flex items-center justify-center shadow hover:bg-indigo-700 transition">
                                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                                        </svg>
                                                                    </button>
                                                                    <button @click.stop="$dispatch('open-modal', { type: 'delete-item', id: {{ $story->id }}, programId: {{ $program->id }}, endpoint: '/admin/stories/{{ $story->id }}' })"
                                                            ...
                                                            <div class="story-card-body">
                                                                ...
                                                                @if ($story->link)
                                                                    <a href="{{ $story->link }}" target="_blank" rel="noopener" class="story-card-link" style="color:{{ $c['600'] }}" @click.stop>Read →</a>
                                                                @else
                                                                    <span class="text-xs font-medium mt-1 block opacity-40 italic" style="color:{{ $c['600'] }}">No link set</span>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                    <button @click="$dispatch('open-modal', { type: 'add-story', programId: {{ $program->id }} })" class="story-add-slot">
                                ...
                                <div class="lg:col-span-1"
                                    ...
                                    <div class="flex items-center justify-between mb-3">
                                        ...
                                        <button @click="$dispatch('open-modal', { type: 'add-testimonial', programId: {{ $program->id }} })"
                                    ...
                                    {{-- Testimonial Cards --}}
                                    <div class="testimonials-scroll space-y-3 overflow-y-auto pr-1" style="height: 830px; scrollbar-width: thin; scrollbar-color: #cbd5e1 transparent;">
                                        <template x-for="t in paginated" :key="t.id">
                                            <div class="bg-white rounded-xl p-4 shadow-sm border-2 relative group/testimonial"
                                                ...
                                                {{-- Edit / Delete --}}
                                                <span class="absolute top-2.5 right-2.5 flex gap-1 opacity-0 group-hover/testimonial:opacity-100 transition">
                                                    <button @click="$dispatch('open-modal', { type: 'edit-testimonial', id: t.id, programId: {{ $program->id }}, data: { quote: t.quote, author_name: t.author_name, author_role: t.author_role } })"
                                                    ...
                                                    <button @click="$dispatch('open-modal', { type: 'delete-item', id: t.id, programId: {{ $program->id }}, endpoint: '/admin/testimonials/' + t.id })"
                                        ...
                                        <template x-if="testimonials.length === 0">
                                            <button @click="$dispatch('open-modal', { type: 'add-testimonial', programId: {{ $program->id }} })"
                                                ...
                                                <span class="text-sm font-semibold">Add First Testimonial</span>
                                            </button>
                                        </template>
                                    </div>
                            ...
                            <div class="mt-8 pt-6 border-t border-slate-200 space-y-3">
                                ...
                                @if ($program->is_published && $program->has_draft_changes)
                                    <div class="flex items-center gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-xs text-amber-600 mt-0.5">The public page still shows the last published version. Click <strong>Republish</strong> to push your changes live.</p>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="flex items-center justify-between gap-4">
                                    <div class="flex items-center gap-2.5">
                                        <span class="w-2.5 h-2.5 rounded-full flex-shrink-0 {{ $program->is_published ? 'bg-emerald-500' : 'bg-amber-400' }}"></span>
                                        <p class="text-sm font-semibold {{ $program->is_published ? 'text-emerald-700' : 'text-amber-700' }}">
                                            @if ($program->is_published && $program->has_draft_changes)
                                        ...
                                        @if ($program->is_published && $program->has_draft_changes)
                                            @php
                                                ...
                                                if ($liveQualCount > $snapQualCount)
                                                    $changes[] = ['icon' => 'plus',  'text' => ($liveQualCount - $snapQualCount) . ' qualification(s) added'];
                                                elseif ($liveQualCount < $snapQualCount)
                                                    $changes[] = ['icon' => 'minus', 'text' => ($snapQualCount - $liveQualCount) . ' qualification(s) removed'];
                                                elseif ($snapQualCount > 0) {
                                                    $snapQualJson = collect($snap['qualifications'])->sortBy('id')->values()->toJson();
                                                    $liveQualJson = $program->qualifications->map(fn($q) => ['id'=>$q->id,'type'=>$q->type,'content'=>$q->content])->sortBy('id')->values()->toJson();
                                                    if ($snapQualJson !== $liveQualJson)
                                                        $changes[] = ['icon' => 'edit', 'text' => 'Qualifications edited'];
                                                }
                                                ...
                                                if ($liveStepCount > $snapStepCount)
                                                    $changes[] = ['icon' => 'plus',  'text' => ($liveStepCount - $snapStepCount) . ' step(s) added'];
                                                elseif ($liveStepCount < $snapStepCount)
                                                    $changes[] = ['icon' => 'minus', 'text' => ($snapStepCount - $liveStepCount) . ' step(s) removed'];
                                                elseif ($snapStepCount > 0) {
                                                    $snapSteps = collect($snap['how_to_apply'])->map(fn($s) => ['id'=>$s['id'],'content'=>$s['content'],'link'=>$s['link']??null])->keyBy('id');
                                                    $liveSteps = $program->howToApply->map(fn($s) => ['id'=>$s->id,'content'=>$s->content,'link'=>$s->link])->keyBy('id');
                                                    $editedCount = $liveSteps->filter(fn($s, $id) => isset($snapSteps[$id]) && (
                                                        $snapSteps[$id]['content'] !== $s['content'] ||
                                                        ($snapSteps[$id]['link'] ?? null) !== $s['link']
                                                    ))->count();
                                                    if ($editedCount > 0)
                                                        $changes[] = ['icon' => 'edit', 'text' => $editedCount . ' How to Apply step(s) edited'];
                                                }
                                                ...
                                                if ($liveStoryCount > $snapStoryCount)
                                                    $changes[] = ['icon' => 'plus',  'text' => ($liveStoryCount - $snapStoryCount) . ' story/stories added'];
                                                elseif ($liveStoryCount < $snapStoryCount)
                                                    $changes[] = ['icon' => 'minus', 'text' => ($snapStoryCount - $liveStoryCount) . ' story/stories removed'];
                                                elseif ($snapStoryCount > 0) {
                                                    $snapStories = collect($snap['stories'])->map(fn($s) => ['id'=>$s['id'],'title'=>$s['title'],'link'=>$s['link']??null])->keyBy('id');
                                                    $liveStories = $program->stories->map(fn($s) => ['id'=>$s->id,'title'=>$s->title,'link'=>$s->link])->keyBy('id');
                                                    $editedCount = $liveStories->filter(fn($s, $id) => isset($snapStories[$id]) && (
                                                        $snapStories[$id]['title'] !== $s['title'] ||
                                                        ($snapStories[$id]['link'] ?? null) !== $s['link']
                                                    ))->count();
                                                    if ($editedCount > 0)
                                                        $changes[] = ['icon' => 'edit', 'text' => $editedCount . ' success story/stories edited'];
                                                }
                                                ...
                                                if ($liveTestCount > $snapTestCount)
                                                    $changes[] = ['icon' => 'plus',  'text' => ($liveTestCount - $snapTestCount) . ' testimonial(s) added'];
                                                elseif ($liveTestCount < $snapTestCount)
                                                    $changes[] = ['icon' => 'minus', 'text' => ($snapTestCount - $liveTestCount) . ' testimonial(s) removed'];
                                                elseif ($snapTestCount > 0) {
                                                    $snapTests = collect($snap['testimonials'])->map(fn($t) => ['id'=>$t['id'],'quote'=>$t['quote'],'author_name'=>$t['author_name'],'author_role'=>$t['author_role']??null])->keyBy('id');
                                                    $liveTests = $program->testimonials->map(fn($t) => ['id'=>$t->id,'quote'=>$t->quote,'author_name'=>$t->author_name,'author_role'=>$t->author_role])->keyBy('id');
                                                    $editedCount = $liveTests->filter(fn($t, $id) => isset($snapTests[$id]) && (
                                                        $snapTests[$id]['quote'] !== $t['quote'] ||
                                                        $snapTests[$id]['author_name'] !== $t['author_name'] ||
                                                        ($snapTests[$id]['author_role'] ?? null) !== $t['author_role']
                                                    ))->count();
                                                    if ($editedCount > 0)
                                                        $changes[] = ['icon' => 'edit', 'text' => $editedCount . ' testimonial(s) edited'];
                                                }

                                                if (empty($changes))
                                                    $changes[] = ['icon' => 'edit', 'text' => 'Content updated'];
                                            @endphp
    ...
    <div id="peso-section" class="w-full py-16" style="padding-left: 7.5rem; padding-right: 7.5rem;">
        ...
        @php
            ...
            // Has unpublished changes if something was saved AFTER the last publish (or never published)
        ...
        <div class="bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 rounded-2xl px-8 py-7 shadow-2xl mb-2">
            <div class="flex items-center justify-between">
                        ...
                        <p class="text-slate-300 text-sm md:text-base">
                            @if ($directoryPublished)
                                <span class="inline-flex items-center gap-1.5">
                                    ...
                                    Published · last updated {{ \Cache::get('field_offices_published_at') ? \Carbon\Carbon::parse(\Cache::get('field_offices_published_at'))->diffForHumans() : 'unknown' }}
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1.5">
                ...
                <div class="flex items-center gap-3">
                    ...
                    {{-- Publish button: amber when dirty, green when clean --}}
                    <button
                        x-data="{}"
                        @click="$dispatch('open-modal', { type: 'publish-directory' })"
                        :class="$store.pesoDirectory.hasDraftChanges ? 'bg-amber-500 hover:bg-amber-400' : 'bg-emerald-500 hover:bg-emerald-400'"
                        class="flex items-center gap-2 px-5 py-2.5 text-white font-bold text-sm rounded-xl shadow-lg transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                        </svg>
                        <span x-text="$store.pesoDirectory.hasDraftChanges ? 'Update Publish' : '{{ $directoryPublished ? 'Update Publish' : 'Publish Directory' }}'">
        ...
        function pesoDirectory() {
            return {
                province: '', officeType: '', showType: false, showResults: false,
                openId: null, pesoData: {}, officeTypes: [], search: '', _fuseCache: {},
                async init() {
                ...
                filteredEntries() {
                    ...
                    // Build or reuse a Fuse instance per province+type combination
        ...
        <div id="peso-ajax-container" class="bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden"
                ...
                <div x-ref="typeSection"
                    ...
                    <div class="flex flex-wrap gap-3 w-full">
                        ...
                        <button @click="selectType('ALL')" type="button"
                            ...
                            :style="officeType === 'ALL' ? 'background:#eef2ff; border-color:#6366f1; box-shadow:0 0 0 3px #eef2ff; transform:translateY(-2px);' : 'background:white; border-color:#e2e8f0;'">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" :style="officeType === 'ALL' ? 'color:#6366f1' : 'color:#94a3b8'">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                            </svg>
                            <span class="text-xs font-bold" :style="officeType === 'ALL' ? 'color:#6366f1' : 'color:#64748b'">All Offices</span>
                            <span class="text-xs font-semibold px-2 py-0.5 rounded-full"
                                :style="officeType === 'ALL' ? 'background:white; color:#6366f1; border:1px solid #c7d2fe' : 'background:#f1f5f9; color:#94a3b8; border:1px solid #e2e8f0'"
                                x-text="countFor(province, 'ALL') + ' offices'"></span>
                        </button>
                        {{-- Dynamic type buttons — same icon, auto-color by index --}}
                        ...
                        <p x-show="officeTypes.length === 0" class="text-xs text-slate-400 italic self-center">No types found.</p>
                    </div>
                </div>
                ...
                <div x-ref="resultsSection"
                    ...
                    <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-3 items-start">
                        <template x-for="entry in filteredEntries()" :key="entry.id">
                            <div x-data="pesoCard(entry.id)" class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all duration-200">
                                ...
                                <div x-show="isOpen(entry.id)"
                                    ...
                                    <div class="flex gap-2 pt-1">
                                        <button @click.stop="$dispatch('open-modal', { type: 'edit-peso', id: entry.id, data: { name: entry.name, manager: entry.manager, email: entry.email, address: entry.address, type: entry.type, province: province } })"
                                        ...
                                        <button @click.stop="$dispatch('open-modal', { type: 'delete-peso', id: entry.id })"
    ...
    @php
        ...
        $ctaHasDraft     = $ctaSection && $ctaSection->published_at
                            ? $ctaSection->updated_at->gt($ctaSection->published_at)
                            : (bool) $ctaSection;
    @endphp
    <div id="cta-section-root" class="bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 py-20 mt-16 relative group/cta"
        x-data="{
        ...
        <div class="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover/cta:opacity-100 transition-opacity duration-200">

            {{-- Edit CTA --}}
            <button @click="$dispatch('open-modal', { type: 'edit-cta', data: { title: ctaTitle, subtitle: ctaSubtitle } })"
            ...
            <button @click="publishCta()"
                :disabled="ctaPublishing || (!ctaHasDraft && ctaIsPublished)"
                :class="ctaHasDraft
                    ? 'bg-amber-500 hover:bg-amber-600 text-white'
                    : (ctaIsPublished ? 'bg-white/10 text-white/40 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700 text-white')"
                ...
                {{-- Spinner --}}
                <svg x-show="ctaPublishing" class="w-3.5 h-3.5 animate-spin flex-shrink-0" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
                </svg>
                {{-- Republish icon --}}
                <svg x-show="!ctaPublishing && ctaHasDraft" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                </svg>
                {{-- First publish icon --}}
                <svg x-show="!ctaPublishing && !ctaHasDraft && !ctaIsPublished" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                </svg>
                ...
                <span x-show="!ctaPublishing && !ctaHasDraft && !ctaIsPublished" x-cloak>Publish to Public</span>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" @click.stop>
            ...
            <div class="px-6 py-6">
                ...
                <div x-show="modal.type === 'add-slide' || modal.type === 'edit-slide'" x-cloak class="space-y-4">
                    <div>
                        ...
                        <input type="text" x-model="form.title"
                            :class="formErrors.title ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                            @input="formErrors.title = false"
                            class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                        <p x-show="formErrors.title" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>This field is required.</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p x-show="formErrors.image" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Please upload an image.</p>
                    </div>
                ...
                <div x-show="modal.type === 'add-program' || modal.type === 'edit-program'" x-cloak class="space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Program Name <span class="text-red-500">*</span></label>
                        <input type="text" x-model="form.name"
                            :class="formErrors.name ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                            @input="formErrors.name = false"
                            class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                        <p x-show="formErrors.name" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Program name is required.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Theme Color <span class="text-red-500">*</span></label>
                        <div :class="formErrors.color ? 'rounded-xl ring-2 ring-red-300 p-2 bg-red-50' : ''" class="grid grid-cols-7 gap-2">
                            <template
                                x-for="c in [
                                ...
                                :key="c.name">
                                <button type="button" @click="form.color = c.name; formErrors.color = false" :title="c.label"
                        ...
                        <p x-show="formErrors.color" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Please select a theme color.</p>
                ...
                <div x-show="modal.type === 'add-qualification' || modal.type === 'edit-qualification'" x-cloak
                    ...
                    <div>
                        ...
                        <div id="quill-qualification"
                            :class="formErrors.content ? 'rounded-lg border-2 border-red-500' : 'rounded-lg border border-slate-300'"></div>
                        <p x-show="formErrors.content" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Content cannot be empty.</p>
                ...
                <div x-show="modal.type === 'add-step' || modal.type === 'edit-step'" x-cloak class="space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Step Content <span class="text-red-500">*</span></label>
                        <div id="quill-step"
                            :class="formErrors.content ? 'rounded-lg border-2 border-red-500' : 'rounded-lg border border-slate-300'"></div>
                        <p x-show="formErrors.content" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Content cannot be empty.</p>
                ...
                <template x-if="modal.type === 'add-story' || modal.type === 'edit-story'">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Story Title <span class="text-red-500">*</span></label>
                            <input type="text" x-model="form.title"
                                :class="formErrors.title ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                                @input="formErrors.title = false"
                                class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                            <p x-show="formErrors.title" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Story title is required.</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            "
                                :class="formErrors.image ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300'"
                                class="w-full border rounded-lg px-4 py-2.5 text-sm" />
                            <p x-show="formErrors.image" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Please upload a thumbnail image.</p>
                        </div>
                        <div class="flex gap-3 justify-end pt-4 border-t border-slate-200">
                            <button type="button" @click="modal.open = false"
                                class="px-5 py-2.5 border border-slate-300 text-slate-700 rounded-lg font-semibold text-sm hover:bg-slate-50 transition">Cancel</button>
                ...
                <div x-show="modal.type === 'add-testimonial' || modal.type === 'edit-testimonial'" x-cloak
                    class="space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Quote <span class="text-red-500">*</span></label>
                        <textarea x-model="form.quote" rows="4" placeholder="Enter the testimonial quote..."
                            :class="formErrors.quote ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                        ...
                        <p x-show="formErrors.quote" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Quote is required.</p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Name <span class="text-red-500">*</span></label>
                            <input type="text" x-model="form.author_name" placeholder="e.g. Juan dela Cruz"
                                :class="formErrors.author_name ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                                @input="formErrors.author_name = false"
                                class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                            <p x-show="formErrors.author_name" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Author name is required.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Role / Program</label>
                ...
                <div x-show="modal.type === 'edit-cta'" x-cloak class="space-y-5">
                    <div class="rounded-xl bg-slate-800 px-6 py-5 text-center mb-2">
                        <p class="text-white font-bold text-xl mb-1" x-text="form.title || 'Ready to Start Your Journey?'"></p>
                        <p class="text-slate-300 text-sm" x-text="form.subtitle || 'Your subtitle will appear here.'"></p>
                    </div>
                    <div>
                        ...
                        <input type="text" x-model="form.title"
                            ...
                            class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                        <p x-show="formErrors.title" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Title is required.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Subtitle <span class="text-red-500">*</span></label>
                        <textarea x-model="form.subtitle"
                            :class="formErrors.subtitle ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                            ...
                            rows="3"
                            class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none resize-none"></textarea>
                        <p x-show="formErrors.subtitle" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Subtitle is required.</p>
                    </div>
                    ...
                    <p x-show="modal.error" x-text="modal.error" class="text-xs text-red-500 font-semibold text-center" x-cloak></p>
                </div>
                {{-- END EDIT CTA --}}

                <div x-show="modal.type === 'add-peso' || modal.type === 'edit-peso'" x-cloak class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Office Name <span class="text-red-500">*</span></label>
                            <input type="text" x-model="form.name" placeholder="e.g. PESO MATI CITY"
                                :class="formErrors.name ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                                @input="formErrors.name = false"
                                class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                            <p x-show="formErrors.name" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Office name is required.</p>
                        </div>
                        <div x-data="officeTypeSelector()" x-init="init()">
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Office Type <span class="text-red-500">*</span></label>

                            {{-- Normal dropdown + action buttons --}}
                            <div x-show="mode === 'select'" class="space-y-2">
                                <div class="flex gap-2">
                                    <select x-model="form.type"
                                        :class="formErrors.type ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                                        ...
                                        <template x-for="t in types" :key="t">
                                            <option :value="t" x-text="t" :selected="form.type === t"></option>
                                        </template>
                                    </select>
                                    {{-- Add --}}
                                ...
                                <p x-show="typeError" x-text="typeError" class="text-xs text-red-500 font-semibold" x-cloak></p>
                                <p x-show="formErrors.type && !typeError" class="text-xs text-red-500 font-semibold" x-cloak>Please select an office type.</p>
                            </div>
                            ...
                            <div x-show="mode === 'add'" x-cloak class="space-y-2">
                                <div class="flex gap-2">
                                    <input type="text" x-model="inputName"
                                        ...
                                        @keydown.escape.prevent="mode = 'select'"
                                        placeholder="e.g. SFO"
                                ...
                                <p class="text-xs text-slate-400">Enter the new type code (e.g. SFO) and press Save.</p>
                                <p x-show="typeError" x-text="typeError" class="text-xs text-red-500 font-semibold" x-cloak></p>
                            </div>

                            {{-- Edit type --}}
                            <div x-show="mode === 'edit'" x-cloak class="space-y-2">
                                <p class="text-xs text-slate-500">Renaming: <strong x-text="form.type"></strong></p>
                                <div class="flex gap-2">
                                    <input type="text" x-model="inputName"
                            ...
                            {{-- Delete confirm --}}
                            <div x-show="mode === 'delete'" x-cloak class="rounded-lg border border-red-200 bg-red-50 p-3 space-y-2">
                                <p class="text-sm text-red-700 font-semibold">Delete type "<span x-text="form.type"></span>"?</p>
                                <p class="text-xs text-red-500">This only removes it from the type list. Existing offices with this type are not affected.</p>
                                <div class="flex gap-2">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Province <span class="text-red-500">*</span></label>
                            <select x-model="form.province"
                                :class="formErrors.province ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                            ...
                            <p x-show="formErrors.province" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Province is required.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Manager / Head Name <span class="text-red-500">*</span></label>
                            <input type="text" x-model="form.manager" placeholder="e.g. Juan dela Cruz"
                                :class="formErrors.manager ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                                @input="formErrors.manager = false"
                                class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                            <p x-show="formErrors.manager" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Manager name is required.</p>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="email" x-model="form.email" placeholder="office@gmail.com"
                                :class="formErrors.email ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                                @input="formErrors.email = false"
                                class="w-full border rounded-lg pl-10 pr-4 py-2.5 text-sm focus:ring-2 outline-none" />
                        </div>
                        <p x-show="formErrors.email" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Email address is required.</p>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <textarea x-model="form.address" rows="2" placeholder="Full address..."
                                :class="formErrors.address ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-300 focus:ring-indigo-400'"
                        ...
                        <p x-show="formErrors.address" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Address is required.</p>
                    </div>
                    <div class="flex gap-3 justify-end pt-4 border-t border-slate-200">
                        <button type="button" @click="modal.open = false"
                            class="px-5 py-2.5 border border-slate-300 text-slate-700 rounded-lg font-semibold text-sm hover:bg-slate-50 transition">Cancel</button>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-slate-500 text-sm">This will push all your changes live to the public page.</p>
                            </div>
                        </div>
                        ...
                        <div x-show="$store.pesoDirectory.changeLog.length > 0"
                            ...
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2.5">Changes to be published</p>
                            <ul class="space-y-2">
                                <template x-for="(entry, i) in $store.pesoDirectory.changeLog" :key="i">
                                    <li class="flex items-center gap-2 text-xs">
                                        ...
                                        <span class="text-slate-700 font-medium" x-text="entry.label"></span>
                                        <span class="text-slate-400" x-text="'(' + entry.type + ', ' + entry.province + ')'"></span>
                                        <span class="text-slate-400 ml-auto flex-shrink-0" x-text="entry.time"></span>
                                    </li>
                                </template>
                            </ul>
                        </div>
                        ...
                        <div class="flex gap-3 justify-end pt-1 border-t border-slate-100">
                            <button type="button" @click="modal.open = false"
                                class="px-5 py-2.5 border border-slate-300 text-slate-700 rounded-lg font-semibold text-sm hover:bg-slate-50 transition">Cancel</button>
                            <button type="button" @click="submitPublishDirectory()" :disabled="modal.loading"
                ...
                <template x-if="modal.type === 'republish-program'">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-slate-800 font-bold text-lg leading-tight">Republish "<span x-text="modal.programName"></span>"?</p>
                                <p class="text-slate-500 text-xs mt-0.5">These changes will go live on the public page.</p>
                            </div>
                        </div>

                        {{-- Changelog --}}
                        <div class="bg-slate-50 rounded-xl border border-slate-200 px-4 py-3 space-y-2">
                            <p class="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">What's changing</p>
                                ...
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <template x-if="change.icon === 'plus'">
                                        <span class="w-5 h-5 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0">
                                            <svg class="w-3 h-3 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4"/></svg>
                                        </span>
                                    </template>
                                    {{-- Minus --}}
                                    <template x-if="change.icon === 'minus'">
                                        <span class="w-5 h-5 rounded-full bg-red-100 flex items-center justify-center flex-shrink-0">
                                            <svg class="w-3 h-3 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M20 12H4"/></svg>
                                        </span>
                                    </template>
                                    {{-- Edit --}}
                                    <template x-if="change.icon === 'edit'">
                                        <span class="w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                                            <svg class="w-3 h-3 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        </span>
                                    </template>
                                    {{-- Tag / name --}}
                                    <template x-if="change.icon === 'tag'">
                                        <span class="w-5 h-5 rounded-full bg-indigo-100 flex items-center justify-center flex-shrink-0">
                                            <svg class="w-3 h-3 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z"/></svg>
                                        </span>
                                    </template>
                                    {{-- Doc / description --}}
                                    ...
                                    <template x-if="change.icon === 'color'">
                                        <span class="w-5 h-5 rounded-full bg-pink-100 flex items-center justify-center flex-shrink-0">
                                    ...
                                    <template x-if="change.icon === 'image'">
                                        <span class="w-5 h-5 rounded-full bg-purple-100 flex items-center justify-center flex-shrink-0">
                                    ...
                                    <template x-if="change.icon === 'text'">
                                        <span class="w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center flex-shrink-0">
                                            <svg class="w-3 h-3 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7"/></svg>
                                        </span>
                                    </template>
                        ...
                        <div class="flex gap-3 justify-end pt-1">
                            <button type="button" @click="modal.open = false"
                                class="px-5 py-2.5 border border-slate-300 text-slate-700 rounded-lg font-semibold text-sm hover:bg-slate-50 transition">Not yet</button>
        ...
        function adminPage() {
            ...
            return {
                ...
                showSuccess(title, message) {
                    ...
                    this.modal.loading = false;
                    this.fieldErrors = {};
                    window.dispatchEvent(new CustomEvent('show-success-modal', { detail: { title, message } }));
                },
                clearFieldErrors() {
                    this.fieldErrors = {};
                ...
                async submitQualification() {
                    ...
                    if (!this.form.content?.trim() || this.form.content === '<p><br></p>') {
                        this.formErrors.content = true; return;
                    }
                    this.modal.loading = true;
                    this.modal.error = null;
                    ...
                    if (!isEdit && listItems.length > 1) {
                        for (const li of listItems) {
                            ...
                            if (!res.success) { this.fail(res.message); return; }
                        }
                        this.done();
                        return;
                    }
                ...
                async submitDelete() {
                    ...
                    if (res.success) {
                        this.done();
                        window.dispatchEvent(new CustomEvent('show-success-modal', { detail: { message: 'Item deleted successfully.' } }));
                    } else {
                    ...
                    if (res.success) {
                        ...
                        if (pesoState) {
                            ...
                            if (isEdit) {
                                ...
                                pesoState.pesoData[prov] = [
                                    ...pesoState.pesoData[prov],
                                    { id: res.id ?? Date.now(), name: body.name, type: body.office_type, manager: body.manager_name, email: body.email, address: body.address }
                                ];
                            }
                        }
                        ...
                        window.dispatchEvent(new CustomEvent('show-success-modal', {
                            detail: {
                                ...
                                message: isEdit
                                    ? escapeText(body.name) + ' has been updated successfully.'
                                    : escapeText(body.name) + ' has been added to ' + escapeText(prov) + '.'
                            }
                        }));
                    } else {
                        this.fail(res.message);
                ...
                async destroyFieldOffice() {
                    ...
                    if (res.success) {
                        ...
                        this.modal.open = false;
                        window.dispatchEvent(new CustomEvent('show-success-modal', { detail: { title: 'Office Deleted', message: escapeText(deletedName) + ' has been removed from the directory.' } }));
                    } else {
                        this.fail(res.message);
                    }
                },
                ...
                async submitPublishDirectory() {
                    ...
                    if (res.success) {
                        ...
                        this.modal.open = false;
                        window.dispatchEvent(new CustomEvent('show-success-modal', { detail: { title: 'Directory Published!', message: 'The PESO / JPO Directory is now live and visible to the public.' } }));
                    } else {
    ...
    function refreshProgramBody(programId) {
        ...
        if (!body) { refreshProgramsContainer(); return; }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    window.dispatchEvent(new CustomEvent('cta-updated', {
                        detail: { title: pending.title, subtitle: pending.subtitle }
                    }));
                    ...
                    if (alpineData) {
                        ...
                        alpineData.message = 'Changes saved. Click \'Publish to Public\' or \'Update Published\' to make them live.';
                        alpineData.open    = true;
                    }
                } else {
    ...
    {{-- ===== CTA CONFIRM MODAL ===== --}}
    <div id="ctaConfirmModal" class="hidden fixed inset-0 z-[1200] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 px-6 pt-8 pb-6 text-center">
                <div class="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-3">
                    <svg class="w-9 h-9 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="px-6 py-6 text-center">
                <p class="text-slate-700 text-sm font-medium mb-6">
                    You're about to save changes to the CTA section. Use <strong>Update Published</strong> to push changes to the public page.
                </p>
    ...
    {{-- ===== SUCCESS MODAL ===== --}}
    <div id="ctaSuccessModal" x-data="{ open: false, title: 'Success!', message: '' }"
        x-show="open"
        x-cloak
