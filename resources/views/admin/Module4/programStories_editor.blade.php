...
<head>
    ...
    @vite('resources/js/admin/Module4/program-stories-editor.js')

    {{-- Alpine must be LAST — reads x-data functions already defined above --}}
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" @click.stop>
                    ...
                    <div class="px-6 py-6">
                        ...
                        <div x-show="modal.type === 'add-slide' || modal.type === 'edit-slide'" x-cloak
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-1">Full Story Link
                                    <span class="text-red-500">*</span></label>
                                <input type="url" x-model="form.link" placeholder="https://..."
                                    :class="formErrors.link ? 'border-red-500 ring-2 ring-red-200' :
                                        'border-slate-300 focus:ring-indigo-400'"
                                    @input="formErrors.link = false"
                                    class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                                <p x-show="formErrors.link" class="mt-1 text-xs text-red-500 font-semibold"
                                    x-cloak>Full story link is required.</p>
                            </div>
                            <div>
                                ...
                                if (!ALLOWED.includes(file.type)) {
                                    showToast('Invalid file type. Please upload a JPG, PNG, WebP, or GIF.', 'warning');
                                    $event.target.value = '';
                                    return;
                                }
                                if (file.size > 20 * 1024 * 1024) {
                                    showToast('Image is too large (max 20 MB).', 'warning');
                                    $event.target.value = '';
                                    return;
                                }
                                const img = new Image();
                        ...
                        <div x-show="modal.type === 'add-program' || modal.type === 'edit-program'" x-cloak
                            ...
                            <div>
                                ...
                                <div id="quill-program" :class="formErrors.description ? 'rounded-lg border-2 border-red-500' : 'rounded-lg border border-slate-300'"></div>
                                <div class="mt-1 text-xs text-slate-400"><span id="quill-program-wordcount">0</span> words</div>
                                <p x-show="formErrors.description" class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Program description is required.</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                if (file.size > 20 * 1024 * 1024) {
                                    showToast('Logo is too large (max 20 MB).', 'warning');
                                    $event.target.value = '';
                                    return;
                                }
                                const img = new Image();
                        ...
                        <div x-show="modal.type === 'add-qualification' || modal.type === 'edit-qualification'" x-cloak
                            class="space-y-4">
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-1">Type <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="url" x-model="form.link" placeholder="https://..."
                                        ...
                                        class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                                    <p x-show="formErrors.link" class="mt-1 text-xs text-red-500 font-semibold"
                                        x-cloak>Story link is required.</p>
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 mb-1">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" x-model="form.author_role"
                                        ...
                                        :class="formErrors.author_role ? 'border-red-500 ring-2 ring-red-200' :
                                            'border-slate-300 focus:ring-indigo-400'"
                                        @input="formErrors.author_role = false"
                                        class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 outline-none" />
                                    <p x-show="formErrors.author_role"
                                        class="mt-1 text-xs text-red-500 font-semibold" x-cloak>Role / Program is
                                        required.</p>
                                </div>
                            </div>
                            <div class="flex gap-3 justify-end pt-4 border-t border-slate-200">
