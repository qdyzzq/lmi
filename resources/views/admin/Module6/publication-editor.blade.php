...
<body class="bg-slate-100 min-h-screen" x-data="lmiAdminPage()">
        ...
        <div class="flex-1 flex flex-col overflow-y-auto" x-ref="mainContent">
            ...
            <div class="relative w-full h-[500px] md:h-[700px] lg:h-[900px] overflow-hidden shrink-0">
                <div class="absolute inset-0">
                    <img src="{{ asset('images/carousel-peso/peso-carousel1.webp') }}" alt="LMI Banner"
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200/60 overflow-hidden">
                    ...
                    <div class="px-5 sm:px-6 pb-6 pt-4 flex flex-col gap-4">
                                ...
                                @foreach ($groupData as $groupId => $group)
                                    ...
                                    <template x-if="activeTab === '{{ $groupId }}'">
                                        <div x-data="{
                                            issues: {{ $issuesJson }},
                                            isPublished: {{ $isPublished ? 'true' : 'false' }},
                                            hasDraftChanges: {{ $hasDraft ? 'true' : 'false' }},
                                            activeYear: '',
                                            get years() { return [...new Set(this.issues.map(i => String(i.year)))].sort((a, b) => b.localeCompare(a)); },
                                            get filtered() { return this.activeYear ? this.issues.filter(i => String(i.year) === this.activeYear) : this.issues; },
                                            init() { this.$nextTick(() => { if (this.years.length) this.activeYear = this.years[0]; }); },
                        ...
                        <div class="flex flex-col pt-5 gap-0" x-data="{
                            ...
                            init() { window.addEventListener('weekly-refresh', () => this.refresh()); },
                        }">
                                    ...
                                    <div class="flex-1 flex flex-col justify-between gap-4 px-5 py-6 transition-colors duration-200"
                                        ...
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-slate-100 text-center font-semibold leading-relaxed italic max-w-2xl pt-2"
                                                ...
                                                "Direct insights on weekly hiring trends and vacancy fluctuations in the
                                                Davao region. (Based on PhilJobNet)"
                                            </p>
                                        </div>
                                        <div class="flex items-center gap-3 w-full">
                                            <span class="text-white/40 text-xs">✦</span>
            ...
            <div x-show="modal.open" x-cloak @open-modal.window="openModal($event.detail)"
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" @click.stop>
                    ...
                    <div class="px-6 py-6">
                        ...
                        {{-- ADD / EDIT ISSUE --}}
                        <div x-show="modal.type === 'add-issue' || modal.type === 'edit-issue'" x-cloak
                            class="space-y-4">
                            <div>
                            ...
                            <div>
                                ...
                                <textarea x-model="form.description" rows="3" placeholder="Brief description..."
                                    :class="formErrors.description ? 'border-red-500 ring-2 ring-red-200' :
                                        'border-slate-300 focus:ring-indigo-400'"
                                    @input="formErrors.description = false"
                                    class="w-full border rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-400 outline-none resize-none"></textarea>
                            </div>
                            <div>
            ...
            <div x-data="weeklyModal()" x-show="open" x-cloak @open-weekly-modal.window="openModal($event.detail)"
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" @click.stop>
                    ...
                    <div class="px-6 py-6">
                        ...
                        {{-- ADD WEEKLY --}}
                        <div x-show="type === 'add-weekly'" x-cloak class="space-y-4">
                            ...
                            <div class="flex gap-3 justify-end pt-4 border-t border-slate-200">
                                ...
                                <button type="button" @click="submitAdd()" :disabled="loading"
                                    class="px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-lg font-semibold text-sm transition disabled:opacity-60">
                                    <span x-show="!loading">Save Issue</span>
                        ...
                        {{-- EDIT WEEKLY --}}
                        <div x-show="type === 'edit-weekly'" x-cloak class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 mb-1">Year <span
                                            class="text-red-500">*</span></label>
            ...
            return {
                ...
                async submitAdd() {
                    ...
                    if (!this.form.year) {
                        ...
                        lmiToast('Year is required.', 'warning');
                        return;
                    }
                    if (!this.form.month) {
                        this.errors.month = true;
                        lmiToast('Month is required.', 'warning');
                        return;
                    }
                    if (!this.form.week_number) {
                        this.errors.week_number = true;
                        lmiToast('Week number is required.', 'warning');
                        return;
                    }
                    if (!this.imageFile) {
                        this.errors.image = true;
                        lmiToast('Please select an image.', 'warning');
                        return;
                    }

                    this.loading = true;
                    const res = await fetch('/admin/lmi-weekly', {
                        method: 'POST',
    ...
    <div id="imageLightbox"
        class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 hidden"
        onclick="closeLightbox()">
        <div class="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center"
            onclick="event.stopPropagation()">
            <button onclick="closeLightbox()"
                class="absolute -top-10 right-0 w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition">
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </button>
            <img id="lightboxImg" src="" alt="Weekly Issue"
                class="max-h-[85vh] w-auto rounded-xl shadow-2xl object-contain">
            <p id="lightboxCaption" class="text-white/70 text-xs mt-3"></p>
        </div>
    </div>
