...
async function lmiRefreshAllGroups() {
    try {
        ...
        Object.keys(data).forEach(gId => {
            ...
            // ✅ Also update the panel's inner x-data scope
            // The panel renders a child div inside <template x-if>
            // Find it by querying inside #annual-panel for a div that has Alpine data
            const panel = document.getElementById('annual-panel');
            if (panel) {
                // The rendered template creates a direct child div with its own x-data
                const panelInner = panel.querySelector(':scope > div');
                if (panelInner) {
                    const panelState = Alpine.$data(panelInner);
                    if (panelState && panelState.issues !== undefined) {
                        panelState.issues          = data[gId].issues;
                        panelState.isPublished     = data[gId].is_published;
                        panelState.hasDraftChanges = data[gId].has_draft_changes;

                        const years = [...new Set(panelState.issues.map(i => String(i.year)))].sort((a, b) => b.localeCompare(a));
                        if (years.length && !years.includes(panelState.activeYear)) {
                            panelState.activeYear = years[0];
                        }
                    }
                }
            }
